...
# --- Windows: 修复 torch DLL 加载（适配 ai_dev conda 环境）---
...
class Evaluator:
    ...
    def evaluate_predictions(
        ...
        # np.trapz 在 NumPy 2.0 中已更名为 np.trapezoid
        area = np.trapezoid(seen_accuracy, unseen_accuracy) if hasattr(np, 'trapezoid') else np.trapz(seen_accuracy, unseen_accuracy)
...
# 多模板提示集成（PLID/CLIP 文献标准技术，+0.5~1 AUC，零训练成本）
# 10 个模板覆盖不同语言表达，CLIP 编码后平均 → 更稳健的文本表示
PROMPT_TEMPLATES = [
    "a photo of {attr} {obj}",
    "a photo of a {attr} {obj}",
    "a {obj} that is {attr}",
    "a {obj} that looks {attr}",
    "this is a {attr} {obj}",
    "an image of a {attr} {obj}",
    "a {attr} looking {obj}",
    "the {obj} appears {attr}",
    "a {attr} {obj} in a photo",
    "a picture of {attr} {obj}",
]
...
def compute_representations(model, test_dataset, config, device):
    ...
    # ── 多模板集成（--prompt_ensemble 开启）──────────────────────────
    use_ensemble = getattr(config, 'prompt_ensemble', False)
    if use_ensemble:
        print(f"  [提示集成] 使用 {len(PROMPT_TEMPLATES)} 个模板平均文本表示")
        pairs_list = test_dataset.pairs
        all_template_reps = []
        with torch.no_grad():
            for tmpl in PROMPT_TEMPLATES:
                prompts = [tmpl.format(
                    attr=a.replace(".", " ").lower(),
                    obj=o.replace(".", " ").lower())
                    for a, o in pairs_list]
                toks = clip.tokenize(
                    prompts, context_length=config.context_length
                ).to(device)
                batches = np.array_split(toks, max(1, len(toks) // config.text_encoder_batch_size))
                tmpl_rep = torch.Tensor().to(device).type(model.dtype)
                for batch in batches:
                    feat = model.text_encoder(batch, enable_pos_emb=True)
                    feat = feat / feat.norm(dim=-1, keepdim=True)
                    tmpl_rep = torch.cat([tmpl_rep, feat], dim=0)
                all_template_reps.append(tmpl_rep)
        # 平均所有模板，再 L2 归一化
        rep = torch.stack(all_template_reps, dim=0).mean(0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--prompt_ensemble',
        action='store_true',
        help="多模板提示集成：用 10 个模板平均文本表示，无需重训，+0.5~1 AUC"
    )
    ...
    if config.experiment_name == 'clip':
        ...
        soft_embs = torch.load(config.soft_embeddings, weights_only=False)['soft_embeddings']
    ...
    if config.open_world and config.threshold is None:
        ...
        unseen_scores = torch.load(
            feasibility_path,
            map_location='cpu', weights_only=False)['feasibility']
