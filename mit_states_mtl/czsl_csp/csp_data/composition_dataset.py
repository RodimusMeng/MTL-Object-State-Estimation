...
def _winpath(p):
    r"""Windows 上用 \\?\ 扩展路径前缀绕过 260 字符 MAX_PATH 限制。"""
...
class ImageLoader:
    ...
    def __call__(self, img):
        ...
        img = Image.open(_winpath(file)).convert('RGB')
        return img
...
class CompositionDataset(Dataset):
    ...
    def get_split_info(self):
        ...
        for instance in data:
            ...
            # 跳过磁盘上确实缺失的图片（约 0.2%，避免训练中途崩）
            if not os.path.exists(_winpath(img_root + image)):
                n_missing += 1
                continue
        ...
        if n_missing:
            print(f"[CompositionDataset] 跳过磁盘缺失图片 {n_missing} 张")
